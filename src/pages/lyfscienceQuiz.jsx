...
const LyfscienceQuiz = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const response = await fetch('http://localhost:3000/lifesciences');
        if (!response.ok) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      navigate('/LC'); // Navigate to home page on successful submission
    } catch (error) {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen bg-login p-8">
      <h1 className="text-2xl font-bold mb-6">Life Science Quiz</h1>
